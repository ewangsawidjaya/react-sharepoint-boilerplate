...
/**
 * Checks a value based on the type passed in
 * @param  {String}  type  
 * @param  {*}  check 
 * @param  {*}  value 
 * @return {Boolean}       
 */
export function isValidWithType(type, check, value) {
	switch(type) {
        case("Simple"):
        	if(value === check || value === undefined || value === null) {
        		return false;	
        	}
            return true;
        ...
        default:
        	console.warn(`Type of (${type}) was not found and validation could not be verified`);
            return true;
    }
}
...
export function buildSharePointQuery(schema) {
	...
	Object.keys(schema).forEach((key) => {
		...
		else if (schema[key].type === "Lookup") {
			select += `${select !== "" ? ', ' : ''}${key}/ID, ${key}/AnnualSpendTotal, ${key}/AFETypeText`;
			expand += `${expand !== "" ? ', ' : ''}${key}`;
		}
 ...
 * @param {Int} itemId 
 */
export function deleteItem(listName, itemId) {
    return Api.delete(`${APP_CONFIG.site.url}/_api/web/lists/getbytitle('${listName}')/items(${itemId})`).then(function(resp) {
        return resp;
    }).catch(function(ex) {
        return ex;
    })
...
/**
 * Returns the item type of the library provided
 * @param {String} name 
 */
export function getItemTypeForLibraryName(name) {
    return "SP.Data." + name.charAt(0).toUpperCase() + name.split(" ").join("_x0020_").slice(1) + "Item";
}
 ...
 * @param {String} folder 
 */
export function createFolder(library, folder) {
    return Api.post(
        `${APP_CONFIG.site.url}/_api/web/folders`, 
        { '__metadata': { 'type': 'SP.Folder' }, 'ServerRelativeUrl': library + '/' + folder }
    ).then(function (resp) {
        return resp;
    }).catch(function (ex) {
        return ex;
